...
CE to independence_number(x) <= 10^order_automorphism_group(x)
    ...
    25
"""
ce15 = Graph("x??C?O?????A?@G?H??????A?C??EGo?@s?O@?O??@g???CO???CAC??a?@g?????H???????????O??H??G??G??@????OA?OCHCO?YA????????A?O???G?O?@????OOC???@??????MCOC???O??[Q??@???????O??G?P?GO@A?G???A???A@??g???W???@CG_???_@O??????@?O@?AGO?????C??A??F??????@C????A?E@L?????P@??")
ce15.name(new = "ce15")
...
trig_antiprism = Graph("Iw?EthkF?")
trig_antiprism.name(new = "trig_antiprism")
...
A graph where none of the upper or lower bounds in the thesis give the exact value for alpha
    ...
    12
    sage: willis_page37_fig510.size()
    18
"""
...
A graph where none of the upper or lower bounds in the thesis give the exact value for alpha
    ...
    12
    sage: willis_page37_fig511.size()
    18
"""
