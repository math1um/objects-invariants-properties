...
# CE to independence_number(x) <= maximum(matching_number(x), critical_independence_number(x))
ce77 = Graph(r"iF\ZccMAoW`Po_E_?qCP?Ag?OGGOGOS?GOH??oAAS??@CG?AA?@@_??_P??G?SO?AGA??M????SA????I?G?I???Oe?????OO???_S?A??A????ECA??C?A@??O??S?@????_@?_??S???O??")
ce77.name(new = "ce77")
...
ce91 = Graph(r"q?}x{k\FGNCRacDO`_gWKAq?ED?Qc?IS?Da?@_E?WO_@GOG@B@?Cc?@@OW???qO?@CC@?CA@C?E@?O?KK???E??GC?CO?CGGI??@?cGO??HG??@??G?SC???AGCO?KAG???@O_O???K?GG????WCG??C?C??_C????q??@D??AO???S????CA?a??A?G??IOO????B?A???_??")
ce91.name(new = "ce91")
...
ce100 = Graph(r'~?@A~~~~~~z~~~~~~~~^~~~~~z~~~~~~~~~~~~~~~v~~~v~~~~~~~~~~z~~~~~~~~^~~~~~~~~~}\~~}v~^~~}~~^~~~~~~~~~~~~^~~~~~~~~V~~~n~~n~~~~~~}~~|~}~~~~~~~~~~~~~~~~~~~~~vv~|~~~~~~~~~~~~~~~~~z~~w~~~~~~~~~~~~~~~~n~~~|~~~~~~~v~|~~~~~~~~~~}~|~r~V~~~n~~~~~~~~z~~}}~}~~~~vz~~z~~~z}~~~n~~~~~~~~~~~~n~~~~~~~z~~~~~~~~~~~~~~^~~~~~~~~~n~~]~~~~~n~~~}~~~~~~~~~~^~^~~~~}~~~~~~~~~~~z~~~~^~~~~~~w')
...
ce106 = Graph(r"k??????????????????????????????????????F~~~z~~~\~~~~{~^|nvny\^~~Njj~~zo}^yB|z~}h|Vv\Jft]~RlhV~tZMC^~fpvBynshVa~yw~@Tv\IVaJ}tvA\erD|Xx_rijkiIxLx}GE\pZ{yIwW?vV}K")
...
ce119 = Graph(r"\?@CoO_CDOC?G?_?O?CO?_?BO?C???O?Bw??k??D_??O???_???C???A???@????g????")
...
ce122 = Graph(r"~?@B??????????????????????????????????????????????????????????????????????????????????B\jeRlsk[rya~Sdr[HeojcW}xwcX_TgTVa?UhcYC?BTaaSo?}YoCRo@ov@_b?EysEu}?Do`uhm?@Ebfkm??~AHGh_?Awyl{w??TrUZDg??AuxBGO??FJ{LUo??O_IF]g??EVmFSY???`YcP_????l|v`}???Bg_[ge???ERCdO[???APuTc^????NQUWCO???B[QrBO????BMP^A_????eo{v`_????NgB\A_????O[Q@HO????@V_oTC?????FCDLPo????AECLPI?????DqZqCE??????")
...
# CE to independence_number(x) >= -max_common_neighbors(x) + min_degree(x) - 1
ce134 = Graph(r'rQjxS{d~tvtrkz~pIJZ{gflAUdYPfybdzswqqS~i`~EfCmswgpu[zfAxLl\TtJEFlilHnZicmo}ZYJjAluT]d|scS\LgJ[|cs~}TBXxNQnJxSm]}oSMt{\kxUl|UhPZHlz`smizxCPTiNL[Mv|kbKUI}^r}oiAdMLE\^rga{v]z@U]Hb}wupjLh`Yg|Rn|`b[iLNp}Oudo~r_`oFEjTzvw')
...
ce136=Graph(r'OAYRKr?KGW_\k@HWW@GSd')
...
#CE to ((matching_covered)^(is_not_forest))->(is_maximal_triangle_free)
ce138 =	Graph('G@GQSK')
...
#CE to size <= matching_number*max_degree + 1, for maximal triangle free graphs
ce140 = Graph('JBIJKv_DMO_')
...
#CE to  is G and H are graphs with distance matrices Gd and Hd and (Gd)(Hd)(Gd) is similar to (Hd)(Gd)(Hd) then G and H are isomorphic. Conjectured by M. Dahir
ce143_G = Graph(r"K?Bcvb[nBUNO")
ce143_H = Graph(r"K?Bcv`\nFSNO")
ce143_G.name(new="ce143_G")
...
"""
Cameron, Kathie, Murilo VG da Silva, Shenwei Huang, and Kristina
Vušković. "Structure and algorithms for (cap, even hole)-free graphs."
Discrete Mathematics 341, no. 2 (2018): 463-473.
"""

hajos=Graph('F|OYW')
hajos.name(new = "Hajos")
add_to_lists(hajos, graph_objects, all_graphs)
...
k5_minor_triangle_even_hole_free = Graph(']h@GgCC?GCo?A??A?G??A@@???G???C?c??C???_?CO???E??GO???A??@G_???C????W????o')
k5_minor_triangle_even_hole_free.name(new = "k5_minor_triangle_even_hole_free")
add_to_lists(k5_minor_triangle_even_hole_free, graph_objects, all_graphs)
...
#CE to if a graph has alpha=lovasz_theta and the graph has a star center then it is perfect. This graph has alpha=lovasz_theta = 3 but has an induced C5 and is not perfect
ce148=Graph('Fsi^_')
